class DateTime extends Date {
  constructor() {
    ...
    this.period = function (date, addDate = 0) {
      return {
        startTime: `${date.getUTCFullYear()}-${paddingLeft(
          (date.getMonth() + 1).toString(),
          2
        )}-${paddingLeft((date.getDate() + addDate).toString(), 2)} 06:00:00`,
        endTime: `${date.getUTCFullYear()}-${paddingLeft(
          (date.getMonth() + 1).toString(),
          2
        )}-${paddingLeft((date.getDate() + addDate).toString(), 2)} 09:00:00`,
        msg: 0 == addDate ? "今日" : "明日",
      };
    };
  }
...
let p = new Promise((resolve, reject) => {
  ...
  function getWeather(dists, apiKey = APIKey) {
    ...
    fetch(
      `https://opendata.cwb.gov.tw/api/v1/rest/datastore/${dists[0].API.forecastWeather}?Authorization=${apiKey}&locationName=${dists[0].dist}`
    ) // 向 requestURL 發送請求
      ...
      .then((data) => {
        ...
        getData(
          data,
          weatherData.find((x) => x.dist == dists[0].dist)
        );
...
function getData(data, distItem) {
  let tempData = data.records.locations[0].location.find(
    (x) => x.locationName == `${distItem.dist}`
  );
  if (undefined == tempData) return;
  distItem.msg = "ok";
  distItem.data.closelyObsTime = tempData.weatherElement[1].time[0].startTime;
  distItem.data.closelyWeather =
    tempData.weatherElement[1].time[0].elementValue[0].value;
  distItem.data.closelyRainningRate =
    tempData.weatherElement[7].time[0].elementValue[0].value;
  distItem.data.forecastWeather = tempData.weatherElement[1].time.find(
    (x) => x.startTime == peakTimePeriod.startTime
  ).elementValue[0].value;
  distItem.data.forecastRainningRate = tempData.weatherElement[7].time.find(
    (x) => x.startTime == peakTimePeriod.startTime
  ).elementValue[0].value;
  distItem.data.forecastTemperture = tempData.weatherElement[3].time.find(
    (x) => x.dataTime == peakTimePeriod.startTime
  ).elementValue[0].value;
}
